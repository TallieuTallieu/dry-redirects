...
class RedirectLogRepository
{
  ...
  public static function count() {
    try {
      $countQuery = Connection::get()->query_field('SELECT COUNT(*) as count FROM redirects_redirect_log');
  ...
  */
  public static function deleteAllBut($count = 20000) {
    try {
      $result = Connection::get()->db_query('SELECT id FROM redirects_redirect_log ORDER BY id DESC LIMIT 1 OFFSET ' . ($count - 1));
